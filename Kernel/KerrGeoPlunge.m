...
KerrGeoISSOPlunge[a_, PlungeType_  ,Arg_, initCoords_] := Module[{consts, \[Theta]0, z, r0,Mino, z0, \[Phi]0, initConditions, t0, assoc,M=1,\[Chi]INC ,t,RI, r, \[Theta], \[Phi], \[ScriptCapitalE], \[ScriptCapitalL], \[ScriptCapitalQ], R4, RM, RP, kz, Z1, Z2, J,velocity},
	...
	\[ScriptCapitalE] = Values[KerrGeoConstantsOfMotion[a,RI,0,\[Chi]INC]][[1]];
	\[ScriptCapitalL] = Values[KerrGeoConstantsOfMotion[a,RI,0,\[Chi]INC]][[2]];
	\[ScriptCapitalQ] = Values[KerrGeoConstantsOfMotion[a,RI,0,\[Chi]INC]][[3]];];
	...
	consts = {\[ScriptCapitalE], \[ScriptCapitalL], \[ScriptCapitalQ]};
	...
	If[r0>RI,
		Message[KerrGeoISSOPlunge::r0outofbounds,r0,RI];
		Return[$Failed]
		];
...
KerrGeoRealPlungeMino[a_, \[ScriptCapitalE]_, \[ScriptCapitalL]_, \[ScriptCapitalQ]_ , initCoords_] := Module[{consts,assoc,M,Mino,t0,r0,\[Phi]0,kr,hR,hP,hM,\[CapitalUpsilon]t,\[CapitalUpsilon]\[Phi],\[CapitalUpsilon]tr,\[CapitalUpsilon]tz,\[CapitalUpsilon]z,\[CapitalUpsilon]r,\[CapitalUpsilon]\[Phi]r,\[CapitalUpsilon]\[Phi]z,t,r,z, \[Theta], J,\[Phi], R1,R2,R3,R4, RM, RP,ROOTS,RealRoots,ComplexRoots, kz, Z1, Z2},
	...
	MinoR[x_]:= 2 InverseJacobiSN[Sqrt[(-R1+R3) (r-R4)]/Sqrt[(r-R1) (R3-R4)] ,KR]/Sqrt[J (R1-R3) (R2-R4)];
	Mino=Function[{Global`r}, Evaluate[MinoR[Global`r]- MinoR[r0]],Listable];
...
KerrGeoComplexPlungeMino[a_, \[ScriptCapitalE]_, \[ScriptCapitalL]_, \[ScriptCapitalQ]_ , initCoords_] := Module[{consts, assoc,MinoR,\[CapitalUpsilon]r,\[CapitalUpsilon]\[Theta],t0,r0,\[Phi]0,M,D1M,D1P,D2M,D2P,e,b,c,d,A,B,chi,kr,p2,f, t, r,z, \[Theta], J,\[Phi], R1,R2,R3,R4, RM, RP,ROOTS,RealRoots,ComplexRoots,kz,Z1, Z2, AMR,CNR,SNR,DNR,AMZ},
	...
	\[CapitalUpsilon]r = 2 \[Pi] EllipticK[kr^2]/Sqrt[A B (1-\[ScriptCapitalE]^2) ];
...
KerrGeoISSOPlunge::fourcomplexroots = "This function can only currently solve plunging geodesics in the paramater space where we have four finite real roots or two finite real roots and two finite complex roots.";
KerrGeoISSOPlunge::highenergy = "This code code not support plungin solutions with \[ScriptCapitalE]>1, \[ScriptCapitalE]=`1` has been selected ";


KerrGeoPlunge[a_:0.9,{\[ScriptCapitalE]_:0.8,\[ScriptCapitalL]_:0.3,\[ScriptCapitalQ]_:3}, initPhases:{_,_,_}:Automatic,OptionsPattern[]]:=Module[{param, method, RI, DN, RISSOMIN, RISSOMAX, ROOTS, RealRoots,ComplexRoots},
...
ComplexRoots = Select[ROOTS,Not@PossibleZeroQ@Im[#]&];
	If[\[ScriptCapitalE]>=1,  Message[KerrGeoISSOPlunge::highenergy,\[ScriptCapitalE]];
												Return[$Failed]];

	If[Length[ComplexRoots]!=0, Return[KerrGeoComplexPlungeMino[a, \[ScriptCapitalE], \[ScriptCapitalL], \[ScriptCapitalQ], initPhases]]];
	If[Length[ComplexRoots]==2, Return[KerrGeoRealPlungeMino[a, \[ScriptCapitalE], \[ScriptCapitalL], \[ScriptCapitalQ], initPhases]]];
												];
										



KerrGeoISSOPlunge::ValidISSOParam = "Give a valid ISSO paramaterisation, either 1.ISSORadialParam or 2.ISSOIncParam";


KerrGeoISSOPlunge::r0outofbounds = "Intial radius `1` is larger than outer turning radius `2`.";


KerrGeoPlunge[a_:0.9,PlungeType_:"ISSORadialParam",Arg1_:0.8, initCoords:{_,_,_}:Automatic,OptionsPattern[]]:=Module[{param, \[ScriptCapitalE], method, RI, DN, RISSOMIN, RISSOMAX, ROOTS, RealRoots,ComplexRoots},
(*FIXME: add stability check but make it possible to turn it off*)
...
If[PlungeType!="ISSORadialParam",
	If[ PlungeType!= "ISSORadialParam" , Message[KerrGeoISSOPlunge::ValidISSOParam,r0,R2];
		Return[$Failed]
		];
]];
